...
public class HollowFillerItem extends DevBuildingItem {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (pos1 == null) {
            ...
            // Create hollow shell between the two positions
            ...
            // Place blocks only on the outer shell
            for (int x = minX; x <= maxX; x++) {
                for (int y = minY; y <= maxY; y++) {
                    for (int z = minZ; z <= maxZ; z++) {
                        // Check if this is on any face of the cuboid
            ...
            // Clear positions after building
            clearPosition(stack, POS1_KEY);
            clearPosition(stack, POS2_KEY);
            spawnParticles(level, clickedPos);
            return InteractionResult.CONSUME;
